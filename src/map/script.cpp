...
{
#define RETURN_OP_NAME(type)    case type: \
	return # type
	switch (op)
	{
...
const char *parse_line(const char *p)
{
	...
	if (p2 != NULL) { // variable assignment processed so leave the method
...
static void script_check_buildin_argtype(struct script_state *st, int func)
{
	...
	for (idx = 2; script_hasdata(st, idx); idx++)
	{
		...
		if (type == '?' || type == '*') { // optional argument or unknown number of optional parameters ( no types are after this )
			...
			case 'i':
				if (!data_isint(data) && !(data_isreference(data) && (reference_toparam(data) || reference_toconstant(data) || !is_string_variable(name)))) { // int ( params and constants are always int )
...
BUILDIN_FUNC(getnameditem)
{
	...
	if (!itemdb_exists(nameid) /* || itemdb_isstackable(nameid)*/) { //Even though named stackable items "could" be risky, they are required for certain quests.
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
static bool buildin_delitem_search(struct map_session_data *sd, struct item *it, uint8 exact_match, uint8 loc)
{
	...
	for ( ; ; )
	{
		...
		for (i = 0; amount && i < size; i++)
		{
			...
			if (exact_match) {
				if ((exact_match & 0x1) && (itm->identify != it->identify || itm->attribute != it->attribute || memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching exact attributes
					continue;
				}
				if (exact_match & 0x2) {
					uint8 j;
		...
		if (amount == 0 || important == 0) { // either everything was already consumed or no items were skipped
			...
			for (i = 0; amount && i < size; i++)
			{
				...
				if (exact_match) {
					if ((exact_match & 0x1) && (itm->refine != it->refine || itm->identify != it->identify || itm->attribute != it->attribute || memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching attributes
						continue;
					}
					if (exact_match & 0x2) {
						uint8 j;
...
BUILDIN_FUNC(successrefitem)
{
	...
	if (i >= 0) {
		...
		if (sd->inventory.u.items_inventory[i].refine == MAX_REFINE
		    ...
		    && sd->status.char_id == (int)MakeDWord(sd->inventory.u.items_inventory[i].card[2], sd->inventory.u.items_inventory[i].card[3])) { // Fame point system [DracoRPG]
 ...
 *------------------------------------------*/
static int buildin_killmonster_sub_strip(struct block_list *bl, va_list ap) //same fix but with killmonster instead - stripping events from mobs.
{
...
static int buildin_killmonsterall_sub_strip(struct block_list *bl, va_list ap) //Strips the event from the mob if it's killed the old method.
{
	struct mob_data *md;
...
BUILDIN_FUNC(sc_start)
{
	...
	if (tick == 0 && val1 > 0 && type > SC_NONE && type < SC_MAX && status_sc2skill(type) != 0) { // When there isn't a duration specified, try to get it from the skill_db
...
BUILDIN_FUNC(getmapflag)
{
	...
	if (m >= 0) {
		...
		case MF_JEXP:
			script_pushint(st, map[m].adjust.jexp);
			break;

		case MF_BEXP:
			script_pushint(st, map[m].adjust.bexp);
			break;
		...
		case MF_CHANNELAUTOJOIN:
			script_pushint(st, map[m].flag.chmautojoin);
			break;

		case MF_NOUSECART:
			script_pushint(st, map[m].flag.nousecart);
			break;

		case MF_NOITEMCONSUMPTION:
			script_pushint(st, map[m].flag.noitemconsumption);
			break;

		case MF_SUMSTARTMIRACLE:
			script_pushint(st, map[m].flag.nosumstarmiracle);
			break;

		case MF_NOMINEEFFECT:
			script_pushint(st, map[m].flag.nomineeffect);
			break;

		case MF_NOLOCKON:
			script_pushint(st, map[m].flag.nolockon);
			break;

		case MF_NOTOMB:
			script_pushint(st, map[m].flag.notomb);
			break;

		case MF_NOCOSTUME:
			script_pushint(st, map[m].flag.nocostume);
			break;

		case MF_GVG_TE_CASTLE:
			script_pushint(st, map[m].flag.gvg_te_castle);
			break;

		case MF_GVG_TE:
			script_pushint(st, map[m].flag.gvg_te);
			break;

		case MF_HIDEMOBHPBAR:
			script_pushint(st, map[m].flag.hidemobhpbar);
			break;
		...
		case MF_SKILL_DAMAGE:
		{
			...
			{
			case 1:
				ret_val = map[m].adjust.damage.pc;
				break;

			case 2:
				ret_val = map[m].adjust.damage.mob;
				break;

			case 3:
				ret_val = map[m].adjust.damage.boss;
				break;

			case 4:
				ret_val = map[m].adjust.damage.other;
				break;

			case 5:
				ret_val = map[m].adjust.damage.caster;
				break;

			default:
				ret_val = map[m].flag.skill_damage;
				break;
			}
...
BUILDIN_FUNC(setmapflag)
{
	...
	if (m >= 0) {
		...
		case MF_JEXP:
			map[m].adjust.jexp = (val <= 0) ? 100 : val;
			break;

		case MF_BEXP:
			map[m].adjust.bexp = (val <= 0) ? 100 : val;
			break;
		...
		case MF_CHANNELAUTOJOIN:
			map[m].flag.chmautojoin = 1;
			break;

		case MF_NOUSECART:
			map[m].flag.nousecart = 1;
			break;

		case MF_NOITEMCONSUMPTION:
			map[m].flag.noitemconsumption = 1;
			break;

		case MF_SUMSTARTMIRACLE:
			map[m].flag.nosumstarmiracle = 1;
			break;

		case MF_NOMINEEFFECT:
			map[m].flag.nomineeffect = 1;
			break;

		case MF_NOLOCKON:
			map[m].flag.nolockon = 1;
			break;
		...
		case MF_SKILL_DAMAGE:
		{
			...
			{
			case 1:
				map[m].adjust.damage.pc = val;
				break;

			case 2:
				map[m].adjust.damage.mob = val;
				break;

			case 3:
				map[m].adjust.damage.boss = val;
				break;

			case 4:
				map[m].adjust.damage.other = val;
				break;

			case 5:
				map[m].adjust.damage.caster = val;
				break;
			}
...
BUILDIN_FUNC(removemapflag)
{
	...
	if (m >= 0) {
		...
		case MF_CHANNELAUTOJOIN:
			map[m].flag.chmautojoin = 0;
			break;

		case MF_NOUSECART:
			map[m].flag.nousecart = 0;
			break;

		case MF_NOITEMCONSUMPTION:
			map[m].flag.noitemconsumption = 0;
			break;

		case MF_SUMSTARTMIRACLE:
			map[m].flag.nosumstarmiracle = 0;
			break;

		case MF_NOMINEEFFECT:
			map[m].flag.nomineeffect = 0;
			break;

		case MF_NOLOCKON:
			map[m].flag.nolockon = 0;
			break;
		...
		case MF_NOCOSTUME:
			map[m].flag.nocostume = 0;
			break;

		case MF_GVG_TE_CASTLE:
			map[m].flag.gvg_te_castle = 0;
			break;
...
BUILDIN_FUNC(soundeffectall)
{
	...
	if (script_hasdata(st, 8)) { // specified part of map
		const char *mapname = script_getstr(st, 4);
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		{
		case UMOB_SIZE:
			md->base_status->size = (unsigned char)value;
			calc_status           = true;
			break;
		...
		case UMOB_MAXHP:
			md->base_status->max_hp = (unsigned int)value;
		...
		case UMOB_RACE:
			md->base_status->race = (unsigned char)value;
			calc_status           = true;
			break;
	...
	case BL_HOM:
		...
		{
		case UHOM_SIZE:
			hd->base_status.size = (unsigned char)value;
			calc_status          = true;
			break;
		...
		case UHOM_MAXHP:
			hd->base_status.max_hp = (unsigned int)value;
			status_set_maxhp(bl, (unsigned int)value, 0);
			break;
		...
		case UHOM_MAXSP:
			hd->base_status.max_sp = (unsigned int)value;
		...
		case UHOM_HUNGER:
			...
			clif_send_homdata(map_charid2sd(hd->homunculus.char_id), SP_HUNGRY, hd->homunculus.hunger);
			break;

		case UHOM_INTIMACY:
			hom_increase_intimacy(hd, (unsigned int)value);
			clif_send_homdata(map_charid2sd(hd->homunculus.char_id), SP_INTIMATE, hd->homunculus.intimacy / 100);
			break;
		...
		case UHOM_RACE:
			hd->base_status.race = (unsigned char)value;
			calc_status          = true;
			break;
	...
	case BL_MER:
		...
		{
		case UMER_SIZE:
			mc->base_status.size = (unsigned char)value;
			calc_status          = true;
			break;
		...
		case UMER_MAXHP:
			mc->base_status.max_hp = (unsigned int)value;
			status_set_maxhp(bl, (unsigned int)value, 0);
			break;
		...
		case UMER_RACE:
			mc->base_status.race = (unsigned char)value;
			calc_status          = true;
			break;
	...
	case BL_ELEM:
		...
		{
		case UELE_SIZE:
			ed->base_status.size = (unsigned char)value;
			calc_status          = true;
			break;
		...
		case UELE_MAXHP:
			ed->base_status.max_hp = (unsigned int)value;
			status_set_maxhp(bl, (unsigned int)value, 0);
			break;
		...
		case UELE_MAXSP:
			ed->base_status.max_sp = (unsigned int)value;
			status_set_maxsp(bl, (unsigned int)value, 0);
			break;
		...
		case UELE_RACE:
			ed->base_status.race = (unsigned char)value;
			calc_status          = true;
			break;

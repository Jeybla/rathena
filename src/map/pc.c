...
void pc_reg_received(struct map_session_data *sd)
{
	...
	if ((sd->class_ & MAPID_BASEMASK) == MAPID_TAEKWON) { // Better check for class rather than skill to prevent "skill resets" from unsetting this
...
void pc_bonus2(struct map_session_data *sd, int type, int type2, int val)
{
	...
	case SP_CASTRATE:    // bonus2 bCastrate,sk,n;
		if (sd->state.lr_flag == 2)
			break;
		ARR_FIND(0, ARRAYLENGTH(sd->skillcastrate), i, sd->skillcastrate[i].id == 0 || sd->skillcastrate[i].id == type2);
		if (i == ARRAYLENGTH(sd->skillcastrate)) { //Better mention this so the array length can be updated. [Skotlex]
...
int pc_dead(struct map_session_data *sd, struct block_list *src)
{
	...
	if (battle_config.death_penalty_type
		...
		if ((battle_config.death_penalty_maxlv & 1 || !pc_is_maxbaselv(sd)) && base_penalty > 0) {
			...
			{
			case 1:
				base_penalty = (uint32)(pc_nextbaseexp(sd) * (base_penalty / 10000.));
				break;

			case 2:
				base_penalty = (uint32)(sd->status.base_exp * (base_penalty / 10000.));
				break;
			}
...
short pc_maxparameter(struct map_session_data *sd, enum e_params param)
{
	...
	if ((idx = pc_class2idx(pc_mapid2jobid(class_, sd->status.sex))) >= 0) {
		...
		{
		case PARAM_STR:
			max_param = job_info[idx].max_param.str;
			break;

		case PARAM_AGI:
			max_param = job_info[idx].max_param.agi;
			break;

		case PARAM_VIT:
			max_param = job_info[idx].max_param.vit;
			break;

		case PARAM_INT:
			max_param = job_info[idx].max_param.int_;
			break;

		case PARAM_DEX:
			max_param = job_info[idx].max_param.dex;
			break;

		case PARAM_LUK:
			max_param = job_info[idx].max_param.luk;
			break;
		}
		if (max_param > 0)
			return max_param;

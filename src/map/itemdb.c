...
static bool itemdb_parse_dbrow(char **str, const char *source, int line, int scriptopt)
{
	...
	if (id->type < 0 || id->type == IT_UNKNOWN || id->type == IT_UNKNOWN2 || (id->type > IT_SHADOWGEAR && id->type < IT_CASH) || id->type >= IT_MAX) { // catch invalid item types

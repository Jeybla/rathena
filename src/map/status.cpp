...
#define set_sc_with_vfx(skill, sc, icon, flag)    set_sc((skill), (sc), (icon), (flag)); if ((icon) < SI_MAX) \
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	case SC_PYREXIA:
	case SC_OBLIVIONCURSE:
	case SC_LEECHESEND: // It doesn't stack or even renew
	{
		int i = SC_TOXIN;

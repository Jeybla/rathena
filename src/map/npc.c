...
int npc_isnear_sub(struct block_list *bl, va_list args)
{
	...
	if (skill_id > 0) { //If skill_id > 0 that means is used for INF2_NO_NEARNPC [Cydh]
		...
		if (idx > 0 && skill_db[idx]->unit_nonearnpc_type) {
			while (1)
			{
				if (skill_db[idx]->unit_nonearnpc_type & 1 && nd->subtype == NPCTYPE_WARP)
					break;
				if (skill_db[idx]->unit_nonearnpc_type & 2 && nd->subtype == NPCTYPE_SHOP)
					break;
				if (skill_db[idx]->unit_nonearnpc_type & 4 && nd->subtype == NPCTYPE_SCRIPT)
					break;
				if (skill_db[idx]->unit_nonearnpc_type & 8 && nd->subtype == NPCTYPE_TOMB)
					break;
				return 0;
			}
		}
...
int npc_touch_areanpc2(struct mob_data *md)
{
	...
	for (i = 0; i < map[m].npc_num; i++)
	{
		...
		if (x >= map[m].npc[i]->bl.x - xs && x <= map[m].npc[i]->bl.x + xs && y >= map[m].npc[i]->bl.y - ys && y <= map[m].npc[i]->bl.y + ys) { // In the npc touch area
			switch (map[m].npc[i]->subtype)
			{
			case NPCTYPE_WARP:
...
static int npc_selllist_sub(struct map_session_data *sd, int n, unsigned short *item_list, struct npc_data *nd)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (itemdb_isequip(sd->inventory.u.items_inventory[idx].nameid)) { // process equipment based information into the arrays

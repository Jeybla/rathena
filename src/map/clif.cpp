...
void clif_skill_warppoint(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv, unsigned short map1, unsigned short map2, unsigned short map3, unsigned short map4)
{
	...
	if (map1 == (unsigned short)-1)
		strcpy(WFIFOCP(fd, 4), "Random");
	else // normal map name
	if (map1 > 0)
		mapindex_getmapname_ext(mapindex_id2name(map1), WFIFOCP(fd, 4));
	if (map2 > 0)
		mapindex_getmapname_ext(mapindex_id2name(map2), WFIFOCP(fd, 20));
	if (map3 > 0)
		mapindex_getmapname_ext(mapindex_id2name(map3), WFIFOCP(fd, 36));
	if (map4 > 0)
		mapindex_getmapname_ext(mapindex_id2name(map4), WFIFOCP(fd, 52));
...
void clif_spiritball(struct block_list *bl)
{
	...
	switch (bl->type)
	{
	case BL_PC : WBUFW(buf, 6) = sd->spiritball;
		break;

	case BL_HOM : WBUFW(buf, 6) = hd->homunculus.spiritball;
		break;
	}
...
void clif_guild_message(struct guild *g, uint32 account_id, const char *mes, int len) // TODO: account_id is not used, candidate for deletion? [Ai4rei]
{
	struct map_session_data *sd;
	uint8                   buf[256];
...
/// 01af <num>.W
void clif_parse_ChangeCart(int fd, struct map_session_data *sd) // TODO: State tracking?
{
	int type;
...
/// 01bb <char name>.24B
void clif_parse_GMShift(int fd, struct map_session_data *sd) // FIXME: remove is supposed to receive account name for clients prior 20100803RE
{
	char *player_name;
...
/// 01bd <char name>.24B
void clif_parse_GMRecall(int fd, struct map_session_data *sd) // FIXME: recall is supposed to receive account name for clients prior 20100803RE
{
	char *player_name;
	char command [NAME_LENGTH + 8];
...
void clif_parse_Auction_setitem(int fd, struct map_session_data *sd)
{
	...
	if ((item = itemdb_exists(sd->inventory.u.items_inventory[idx].nameid)) != NULL && !(item->type == IT_ARMOR || item->type == IT_PETARMOR || item->type == IT_WEAPON || item->type == IT_CARD || item->type == IT_ETC || item->type == IT_SHADOWGEAR)) { // Consumable or pets are not allowed
...
void clif_parse_Auction_register(int fd, struct map_session_data *sd)
{
	...
	if ((item = itemdb_exists(sd->inventory.u.items_inventory[sd->auction.index].nameid)) == NULL) { // Just in case
		clif_Auction_message(fd, 2); // The auction has been canceled
		return;
	}
...
void clif_rental_time(int fd, unsigned short nameid, int seconds) // '<ItemName>' item will disappear in <seconds/60> minutes.
{
...
void clif_rental_expired(int fd, int index, unsigned short nameid) // '<ItemName>' item has been deleted from the Inventory
{

	...
	if (sc && sc->count) {  // SC_* that reduce damage to 0.
		...
		if ((sce = sc->data[SC_AUTOGUARD]) && flag & BF_WEAPON && !(skill_get_nk(skill_id) & NK_NO_CARDFIX_ATK) && rnd() % 100 < sce->val2) {
			...
			if (sce_d && (d_bl = map_id2bl(sce_d->val1))
			    ...
			    && check_distance_bl(bl, d_bl, sce_d->val3)) { //If player is target of devotion, show guard effect on the devotion caster rather than the target
...
static struct Damage battle_calc_element_damage(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	return wd;
} // battle_calc_element_damage

#define ATK_RATE(damage, damage2, a)           do { int64 rate_ = (a); (damage) = (damage) * rate_ / 100; if (is_attack_left_handed(src, skill_id)) \
							    (damage2) = (damage2) * rate_ / 100; }                                                  \
        while (0);
#define ATK_RATE2(damage, damage2, a, b)       do { int64 rate_ = (a), rate2_ = (b); (damage) = (damage) * rate_ / 100; if (is_attack_left_handed(src, skill_id)) \
...
#define ATK_ADDRATE(damage, damage2, a)        do { int64 rate_ = (a); (damage) += (damage) * rate_ / 100; if (is_attack_left_handed(src, skill_id)) \
							    (damage2) += (damage2) * rate_ / 100; }                                                  \
        while (0);
#define ATK_ADDRATE2(damage, damage2, a, b)    do { int64 rate_ = (a), rate2_ = (b); (damage) += (damage) * rate_ / 100; if (is_attack_left_handed(src, skill_id)) \
							    (damage2) += (damage2) * rate2_ / 100; }                                                               \
        while (0);
//Adds an absolute value to damage. 100 = +100 damage
#define ATK_ADD(damage, damage2, a)            do { int64 rate_ = (a); (damage) += rate_; if (is_attack_left_handed(src, skill_id)) \
							    (damage2) += rate_; }                                                   \
        while (0);
#define ATK_ADD2(damage, damage2, a, b)        do { int64 rate_ = (a), rate2_ = (b); (damage) += rate_; if (is_attack_left_handed(src, skill_id)) \
							    (damage2) += rate2_; }                                                                \
        while (0);

#ifdef RENEWAL
	#define RE_ALLATK_ADD(wd, a)           do { int64 a_ = (a); ATK_ADD((wd).statusAtk, (wd).statusAtk2, a_); ATK_ADD((wd).weaponAtk, (wd).weaponAtk2, a_); ATK_ADD((wd).equipAtk, (wd).equipAtk2, a_); ATK_ADD((wd).masteryAtk, (wd).masteryAtk2, a_); } \
        while (0);
	#define RE_ALLATK_RATE(wd, a)          do { int64 a_ = (a); ATK_RATE((wd).statusAtk, (wd).statusAtk2, a_); ATK_RATE((wd).weaponAtk, (wd).weaponAtk2, a_); ATK_RATE((wd).equipAtk, (wd).equipAtk2, a_); ATK_RATE((wd).masteryAtk, (wd).masteryAtk2, a_); } \
        while (0);
	#define RE_ALLATK_ADDRATE(wd, a)       do { int64 a_ = (a); ATK_ADDRATE((wd).statusAtk, (wd).statusAtk2, a_); ATK_ADDRATE((wd).weaponAtk, (wd).weaponAtk2, a_); ATK_ADDRATE((wd).equipAtk, (wd).equipAtk2, a_); ATK_ADDRATE((wd).masteryAtk, (wd).masteryAtk2, a_); } \
        while (0);
#else
...
#define DAMAGE_DIV_FIX(dmg, div)           { if ((div) < 0) { (div) *= -1; (dmg) /= (div); } \
		                             (dmg) *= (div); }
#define DAMAGE_DIV_FIX2(dmg, div)          { if ((div) > 1) \
						     (dmg) *= div; }
#define DAMAGE_DIV_FIX_RENEWAL(wd, div)    do { int div_ = (div); DAMAGE_DIV_FIX2((wd).statusAtk, div_); DAMAGE_DIV_FIX2((wd).weaponAtk, div_); DAMAGE_DIV_FIX2((wd).equipAtk, div_); DAMAGE_DIV_FIX2((wd).masteryAtk, div_); } \
        while (0);
/*================================================= [Playtester]
...
static struct Damage battle_calc_multi_attack(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sd && !skill_id) {  // if no skill_id passed, check for double attack [helvetica]
		...
		if (((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER)
			...
			{
			case 5:
				if (chance < 4) { wd.div_ = 5; break; }         // 3 % chance to attack 5 times.
			case 4:
				if (chance < 7) { wd.div_ = 4; break; }         // 6 % chance to attack 4 times.
			case 3:
				if (chance < 10) { wd.div_ = 3; break; }        // 9 % chance to attack 3 times.
			case 2:
			case 1:
				if (chance < 13) { wd.div_ = 2; break; }         // 12 % chance to attack 2 times.
			}
...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	return skillratio;
} // battle_calc_attack_skill_ratio
...
static int64 battle_calc_skill_constant_addition(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	return atk;
} // battle_calc_skill_constant_addition
...
struct Damage battle_calc_attack_post_defense(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	return wd;
} // battle_calc_attack_post_defense
...
struct Damage battle_calc_attack_left_right_hands(struct Damage wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	return wd;
} // battle_calc_attack_left_right_hands
...
#ifndef RENEWAL
	case ASC_BREAKER:       //Breaker's int-based damage (a misc attack?)
	{
...
#endif
	return wd;
} // battle_calc_weapon_final_atk_modifiers
		...
		if (!status_reflect && sd && sd->bonus.short_weapon_damage_return) {
			...
			if (sc->data[SC_REFLECTSHIELD]) {
				...
				if ((sce_d = sc->data[SC_DEVOTION]) && (d_bl = map_id2bl(sce_d->val1))
				    && ((d_bl->type == BL_MER && ((TBL_MER*)d_bl)->master && ((TBL_MER*)d_bl)->master->bl.id == bl->id)
				        || (d_bl->type == BL_PC && ((TBL_PC*)d_bl)->devotion[sce_d->val2] == bl->id))) { //Don't reflect non-skill attack if has SC_REFLECTSHIELD from Devotion bonus inheritance
...
int battle_check_target(struct block_list* src, struct block_list* target, int flag)
{
	...
	if (map_flag_vs(m)) { //Check rivalry settings.
		...
		if (state & BCT_ENEMY && battle_config.pk_mode && !map_flag_gvg(m) && s_bl->type == BL_PC && t_bl->type == BL_PC) { // Prevent novice engagement on pk_mode (feature by Valaris)

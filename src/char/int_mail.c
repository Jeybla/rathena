...
static bool mail_loadmessage(int mail_id, struct mail_message *msg)
{
	...
	for (i = 0; i < MAIL_MAX_ITEM && SQL_SUCCESS == Sql_NextRow(sql_handle); i++)
	{
		...
		Sql_GetData(sql_handle, 5, &data, NULL);
		msg->item[i].unique_id = strtoull(data, NULL, 10);
		Sql_GetData(sql_handle, 6, &data, NULL);
		...
		for (j = 0; j < MAX_SLOTS; j++)
		{
			...
			msg->item[i].card[j] = atoi(data);
		}

		for (j = 0; j < MAX_ITEM_RDM_OPT; j++)
		{
			Sql_GetData(sql_handle, 7 + MAX_SLOTS + j * 3, &data, NULL);
			msg->item[i].option[j].id = atoi(data);
			Sql_GetData(sql_handle, 8 + MAX_SLOTS + j * 3, &data, NULL);
			msg->item[i].option[j].value = atoi(data);
			Sql_GetData(sql_handle, 9 + MAX_SLOTS + j * 3, &data, NULL);

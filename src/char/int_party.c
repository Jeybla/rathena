...
int inter_party_parse_frommap(int fd)
{
	...
	{
	case 0x3020:
		mapif_parse_CreateParty(fd, RFIFOCP(fd, 4), RFIFOB(fd, 28), RFIFOB(fd, 29), (struct party_member *)RFIFOP(fd, 30));
		break;
	...
	case 0x3024:
		mapif_parse_PartyLeave(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOCP(fd, 14), (enum e_party_member_withdraw)RFIFOB(fd, 14 + NAME_LENGTH));
		break;

...
struct guild* inter_guild_fromsql(int guild_id)
{
	...
	memcpy(g->name, data, zmin(len, NAME_LENGTH));
	Sql_GetData(sql_handle, 1, &data, &len);
	memcpy(g->master, data, zmin(len, NAME_LENGTH));
	...
	if (g->max_member > MAX_GUILD) { // Fix reduction of MAX_GUILD [PoW]
	...
	memcpy(g->mes1, data, zmin(len, sizeof(g->mes1)));
	Sql_GetData(sql_handle, 10, &data, &len);
	memcpy(g->mes2, data, zmin(len, sizeof(g->mes2)));
	...
	g->last_leader_change = atoi(data);
...
int mapif_parse_GuildCastleDataSave(int fd, int castle_id, int index, int value)
{
	...
	return 0;
} // mapif_parse_GuildCastleDataSave
...
int inter_guild_parse_frommap(int fd)
{
	...
	case 0x303A:
		mapif_parse_GuildMemberInfoChange(fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOL(fd, 12), RFIFOW(fd, 16), RFIFOCP(fd, 18), RFIFOW(fd, 2) - 18);
		break;

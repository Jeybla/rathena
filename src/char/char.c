...
int char_memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch) {
	...
	column = 0;
	SqlStmt_BindColumn(stmt, column++, SQLDT_INT,       &item.id,          0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_USHORT,    &item.nameid,      0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_SHORT,     &item.amount,      0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_UINT,      &item.equip,       0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_CHAR,      &item.identify,    0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_CHAR,      &item.refine,      0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_CHAR,      &item.attribute,   0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_UINT,      &item.expire_time, 0, NULL, NULL);
	if( tableswitch == TABLE_INVENTORY )
		SqlStmt_BindColumn(stmt, column++, SQLDT_CHAR,      &item.favorite,    0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_UINT,      &item.bound,       0, NULL, NULL);
	SqlStmt_BindColumn(stmt, column++, SQLDT_UINT64,    &item.unique_id,   0, NULL, NULL);
	for( i = 0; i < MAX_SLOTS; ++i )
		SqlStmt_BindColumn(stmt, column++, SQLDT_USHORT, &item.card[i], 0, NULL, NULL);
	for( i = 0; i < MAX_ITEM_RDM_OPT; ++i ) {
		SqlStmt_BindColumn(stmt, column++, SQLDT_SHORT, &item.option[i].id, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, column++, SQLDT_SHORT, &item.option[i].value, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, column++, SQLDT_CHAR, &item.option[i].param, 0, NULL, NULL);
	}
				...
				else
				{
					...
					if( tableswitch != TABLE_INVENTORY )
						...
						StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%d', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `favorite`='%d', `bound`='%d', `unique_id`='%"PRIu64"'",
					    		tablename, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].favorite, items[i].bound, items[i].unique_id);
	...
	if( tableswitch != TABLE_INVENTORY )
		...
		StringBuf_Printf(&buf, "INSERT INTO `%s` (`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `favorite`, `bound`, `unique_id`", tablename, selectoption);
	for( j = 0; j < MAX_SLOTS; ++j )
